import
  ...
  sync_protocol, request_manager, genesis, quickstart
...
proc initGenesis(node: BeaconNode) {.async.} =
  ...
  of GenesisType.quickStart:
    ...
    var validators = quickStartValidators conf.quickStartTotalValidators

    tailState = initialize_beacon_state_from_eth1(
      quickStartDeposits(validators),
      uint64(1) shl 40, # eth1_timestamp as specified in the spec above
      quickStartEth1Data(), {skipValidation})
